var args = arguments,
    req = requirejs.config({
                               ...
                               packages: [{
                                   name: "amdclean",
                                   location: "node_modules/amdclean",
                                   main: "src/amdclean"
                               }, {
                                   name: "esprima",
                                   location: "node_modules/amdclean/node_modules/esprima",
                                   main: "esprima"
                               }, {
                                   name: "estraverse",
                                   location: "node_modules/amdclean/node_modules/estraverse",
                                   main: "estraverse"
                               }, {
                                   name: "lodash",
                                   location: "node_modules/amdclean/node_modules/lodash",
                                   main: "dist/lodash"
                               }, {
                                   ...
                                   name: "esutils",
                                   location: "converted_modules/escodegen/node_modules/esutils",
                                   main: "lib/utils"
                               }],
                               paths: {
...
req(["amdclean"], function(amdclean) {
    ...
    if (!innerMainPath) {
        if (isNode) {
            ...
            if (process.argv.length < 3) {
                writeError("The file path to clean must be passed as the first argument.");
                exit(1);
            }
    ...
    try {
        ...
        if (fileExists(mapPath)) {
            ...
            // amdclean appears not to read files from the "filePath" property below when running under
            // anything but Node, so we have to read the file and provide it directly.
            ...
            var codeHash = amdclean.clean({
                                              ...
                                              "prefixTransform": function(postNormalizedModuleName,
                                                  ...
                                                  var retVal = parts.join("_").replace(/[^A-Za-z0-9_]/g, "_");
                                                  // main.js in the root directory appears to use the wrong variable, so
                                                  // hack around it here -- also, require calls to the text plugin have
                                                  // a similar issue
                                                  if (retVal === "lib_jscc_main") {
                                                      retVal = "_" + retVal;
                                                  } else {
                                                      var match = /^text_+(?:lib_jscc_)?(.*)$/.exec(retVal);
                                                      if (match) {
                                                          retVal = "text___" + match[1];
                                                      }
                                                  }
                                                  return retVal;
                                              }
                                          });
            // The wrap option does not appear to work correctly when using source maps.  So, we'll throw the global
            // Node require statements in yet another Closure header file instead.
