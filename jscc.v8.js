...
function SYMBOL()
{
	...
	var is_eof;			//Eof-symbol flag (SYM_TERM only)
			...
			for( j = 0; j < item_set[i].lookahead.length; j++ )
			{
				...
				else
				{
					...
					if( ex > 0 )
					{
						...
						//_print( symbols[item_set[i].lookahead[j]].level + " " + productions[ item_set[i].prod ].level );
							...
							|| productions[ item_set[i].prod ].level > 0 )
						{
							...
							{
								//In case of left- or non-associativity, reduce
								if( symbols[item_set[i].lookahead[j]].assoc == ASSOC_LEFT )
									act = -1 * item_set[i].prod;
								else if( symbols[item_set[i].lookahead[j]].assoc == ASSOC_NONE )
									remove_table_entry( states[s].actionrow, item_set[i].lookahead[j] );
							}
							else
							{
								//If symbol precedence is lower production's precedence, reduce
								if( symbols[item_set[i].lookahead[j]].level <
										productions[ item_set[i].prod ].level )
									act = -1 * item_set[i].prod;
							}
...
/* -FUNCTION--------------------------------------------------------------------
	...
	Usage:			Entry function to perform table generation. If all states
					...
					higher precedence. Reduce-reduce conflics cannot be solved
					and are reported.
...
function lalr1_parse_table( debug )
{
	...
	for( i = 0; i < symbols.length; i++ )
	{
		if( symbols[s].kind == SYM_NONTERM )
			
	}
	*/
	
	/*
	item = create_item( symbols[i].prods[0] );
	item.lookahead.push( create_symbol( "EOF", SYM_TERM ).id );
	*/
...
Usage:	Console-based ersion of the JS/CC LALR(1) Parser Generator
		to be executed as V8 code on a local machine.
...
function copyright_info()
{
	...
	info += "Contributions (C) 2007, 2008 by Louis P. Santillan <lpsantil@gmail.com>\n";
