...
(function(root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['./global', './first', './util', './log/log'], factory);
    } else if (typeof module === 'object' && module.exports) {
        module.exports = factory(require('./global'), require('./first'), require('./util'), require('./log/log'));
    } else {
        root.tabgen = factory(root.global, root.first, root.util, root.log);
    }
    ...
    var exports = {
        ...
        add_table_entry: function(row, sym, act) {
            ...
            row.push(global.TableEntry(sym, act));
            return row;
        },
        ...
        lalr1_closure: function(s) {
            ...
            do {
                old_cnt = cnt;
                cnt = exports.close_item(((old_cnt == 0) ? global.states[s].kernel : closure), closure);
            } while (cnt != old_cnt);
                                ...
                                if (ex > 0) {
                                    ...
                                    if (global.symbols[item_set[i].lookahead[j]].level > 0
                                        ...
                                        if (global.symbols[item_set[i].lookahead[j]].level ==
                                            global.productions[item_set[pi].prod].level) {
