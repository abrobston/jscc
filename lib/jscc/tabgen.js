...
(function(root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['require', './global', './first', './util', './log/log', './classes/State', './classes/Item',
                ...
                './enums/MODE_GEN', './enums/EXEC', './debug', './namespace'], factory);
    } else if (typeof module === 'object' && module.exports) {
        module.exports =
            factory(require, require('./global'), require('./first'), require('./util'), require('./log/log'),
                    require('./classes/State'), require('./classes/Item'), require('./classes/TableEntry'),
                    require('./classes/Symbol'), require('./enums/SPECIAL'), require('./enums/ASSOC'),
                    require('./enums/SYM'), require('./enums/MODE_GEN'), require('./enums/EXEC'), require('./debug'),
                    require('./namespace'));
    } else {
        root.tabgen =
            factory(root.require, root.global, root.first, root.util, root.log, root.State, root.Item, root.TableEntry,
                /** @type {function(new:jscc.classes.Symbol, ?SymbolOptions=)} */ (root.Symbol), root.SPECIAL,
                    root.ASSOC,
                    root.SYM, root.EXEC, root.MODE_GEN, root.debug, root.namespace);
    }
                 ...
                 /** jscc.util */ util,
                 /** jscc.log */ log,
                 /** function(new:jscc.classes.State, ?StateOptions=) */ State,
                 /** function(new:jscc.classes.Item, ?ItemOptions=) */ Item,
                 /** function(new:jscc.classes.TableEntry, number, number) */ TableEntry,
                 /** function(new:jscc.classes.Symbol, ?SymbolOptions=) */ Symbol,
                 SPECIAL,
                 ASSOC,
                 SYM,
                 EXEC,
                 MODE_GEN,
                 /** jscc.debug */ debugFunctions,
                 jscc) {
    ...
    function resolveModules() {
        ...
        State = State || require("./classes/State");
        Item = Item || require("./classes/Item");
        TableEntry = TableEntry || require("./classes/TableEntry");
        Symbol = Symbol || require("./Symbol");
    }
                ...
                if (seed[i].dot_offset < global.productions[seed[i].prod].rhs.length) {
                    if (global.symbols[global.productions[seed[i].prod].rhs[seed[i].dot_offset]].kind ==
                        ...
                        for (j = 0;
                            ...
                            if (k == closure.length) {
                                item =
                                    this.create_item(global.symbols[global.productions[seed[i].prod].rhs[seed[i].dot_offset]].prods[j]);
                                closure.push(item);
                                ...
                                if (ex > 0) {
                                    ...
                                    if (global.symbols[item_set[i].lookahead[j]].level > 0
                                        ...
                                        if (global.symbols[item_set[i].lookahead[j]].level ==
                                            ...
                                            if (global.symbols[item_set[i].lookahead[j]].assoc == ASSOC.LEFT) {
                                                // Reduce
                                                act = -1 * item_set[i].prod;
